...
#### 3. 创建用户组 mysql，创建用户 mysql 并将其添加到用户组 mysql 中，并赋予读写权限
...
1. 登录 MySQL01，创建一个数据库，一个表，插入一条数据
...
1. 登录 mysql02，创建用于同步的账户 repl，密码为 123456，并查询 master 状态，记下 file 名称和 posttion 数值，并查询 master 状态
...
priority 99
advert_int 1
unicast_src_ip x.x.x.x //当前主机 ip
unicast_peer {
X.X.X.X // 其他主机 IP
}
authentication {
...
3. /opt/chk_mysql.sh 脚本内容：如果当前主机 mysql 宕机，切换到 bankup，恢复后自动切换回来
...
#### 6. 关于 aws ec2 之间获取不到 keepalived vip 地址问题，并且组播不好用：

解决广播模式：

切换 KeepAliveD 到单播模式，在配置中增加单播机器的 IP：

\`\`\`shell
unicast_src_ip 172.*.*.1 #localIp

unicast*peer { 172.*.\\_.124 #Resource-s
}
\`\`\`

解决获取不到 keepalived vip 地址问题： 使用 ec2 的辅助 ip 地址，在 mysql 出现问题的时候，由 keepalived 监视，动态添加辅助 IP 地址实现，vip 漂移。

1. 用 AWS CLI 来实现 VIP 指向的切换

- 3.1.1 如果使用 private IP 作为 VIP,则：两个服务器配置三个 IP 地址，其中一个是 Secondary private IP，它作为内网的 VIP 使用。这个配置是基础配置。也就是说，如果一个 IP 指向这台服务器，可是这台服务器上的网卡不认为自己拥有这个 IP 是不行的。
- 3.2 在两台服务器上安装亚马逊云自己的客户端工具(awscli) 并配置(Authentication，IP 切换时使用）
- 3.3 在 KeepAliveD 认为自己获取到 Master 权限的时候，调用 awscli 命令让 Virtual IP 实际指向本服务器。这个逻辑是核心逻辑。亚马逊云虽然不允许广播 ARP，但可以用命令行指定网卡（ENI）的 Secondary-private-ip-address。这个命令的具体形式如下：
...
2. 再次编辑 keepalived.conf，加入\`notify\`时执行的脚本：
